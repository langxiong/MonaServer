...
namespace Mona {
    ...
    namespace Events
    {
        struct OnTunnelEstablish : Event<void(PoolBuffer&, const SocketAddress&)> {};
    }

class ProxyProtocol;
class TunnelSession : public Session, public virtual Object {
    /*class ProxySocket : public TCPSocket
    {
    public:
        ProxySocket(const SocketManager& manager, TunnelSession& session) :UDPSocket(manager), _session(session)
        {
    ...
    private:
        ...
        TunnelSession& _session;
    };*/
public:
	TunnelSession(ProxyProtocol& protocol,
        Invoker& invoker, PoolBuffer& pBuffer, const SocketAddress& peerAddress);

    virtual void kill(UInt32 type = NORMAL_DEATH);

private:
    //EventsunnelSession::OnTunnelEstablish::Type _onTunnelEstablish;
